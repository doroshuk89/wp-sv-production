    ...
    function create() {
        foreach ($this->options['post'] as $post_type) {
            if (current_user_can( $this->options['cap'])) {
                add_meta_box($this->options['id'], __($this->options['name'],'svkupe-domain'), array(&$this, 'fill'), $post_type, $this->options['pos'], $this->options['pri']);
            }
        }
    }
    function fill(){
        ...
        foreach ( $this->options['args'] as $param ) {
            if (current_user_can( $param['cap'])) {
                ?><tr><?php
                if(!$value = get_post_meta($post->ID, $this->prefix .$param['id'] , true)) $value = '';
                switch ( $param['type'] ) {
                    case 'text':{ ?>
                        <th scope="row"><label for="<?php echo $this->prefix .$param['id'] ?>"><?php _e($param['title'],'svkupe-domain'); ?></label></th>
                        <td>
                            <input name="<?php echo $this->prefix .$param['id'] ?>" type="<?php echo $param['type'] ?>" id="<?php echo $this->prefix .$param['id'] ?>" value="<?php echo $value ?>" placeholder="<?php  _e($param['placeholder'], 'svkupe-domain'); ?>" class="regular-text" /><br />
                            <span class="description"><?php _e($param['desc'],'svkupe-domain'); ?></span>
                        </td>
                        <?php
                        break;
                    }
        ...
        foreach ( $this->options['args'] as $param ) 
        {
            if ( current_user_can( $param['cap'] ) ) {
                if ( isset( $_POST[ $this->prefix . $param['id'] ] ) && trim( $_POST[ $this->prefix . $param['id'] ] ) ) {
                    ...
                    //Установить значение по умолчанию, используется для сортировки на странице списка записей 
                } else if (isset($param['def'])) {
                        update_post_meta( $post_id, $this->prefix . $param['id'], trim($param['def']));
                } else {
